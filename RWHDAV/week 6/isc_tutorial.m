...
for rr = 1:length(rois)
    ...
    for nn = 1:length(names)
        funcnames{nn} = fullfile(basepath,'subjects',names{nn},condname,'trans_filtered_func_data.nii');
    end
...
clc%% plot lagged corrs for every subject for this ROI
...
for i = 1:length(roin)
    roicorrpath = fullfile(basepath,'intersubj','roicorr'); 
    roicorr_file = [roin{i} '_' condname '_roicorr.mat'];
    r1 = load(fullfile(roicorrpath,roicorr_file));
    subplot(1,3,i)
    imagesc(corr(r1.roitc))
    colorbar
    caxis([-0.5 0.5])
    title(['Pairwaise corr for ',roin{i}])
end
